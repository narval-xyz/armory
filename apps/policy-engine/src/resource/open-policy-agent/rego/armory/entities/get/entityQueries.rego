...
accountByAddress(address) := account if {
	account := {account |
		account := data.entities.accounts[_]
		case.equalsIgnoreCase(account.address, address)
	}[_]
}
...
groupsByAccount(accountId) := groups if {
	groups := {group.id |
		...
		case.findCaseInsensitive(accountId, group.accounts)
	}
} else := null
...
## RETURNS {}
groupsByUser(userId) := groups if {
	groups := {group.id |
		group := data.entities.userGroups[_]
		case.findCaseInsensitive(userId, group.users)
	}
} else := null
