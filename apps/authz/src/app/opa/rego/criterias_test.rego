...
test_check_principal_id { 
   check_principal_id({"test-bob-uid", "test-alice-uid"}) 
        with input as request
        with data.entities as entities
}

test_check_principal_role { 
    check_principal_role({"root", "admin"}) 
        with input as request
        with data.entities as entities
}
...
test_check_wallet_id {
    check_wallet_id({"eip155:eoa:0xddcf208f219a6e6af072f2cfdc615b2c1805f98e"}) 
        with input as request
        with data.entities as entities
}

test_check_wallet_groups {
    check_wallet_groups({"test-wallet-group-one-uid", "test-wallet-group-two-uid"}) 
        with input as request
        with data.entities as entities
}

test_check_wallet_chain_id {
    check_wallet_chain_id({"1", "137"}) 
        with input as request
        with data.entities as entities
}
...
test_check_source_address {
    check_source_address({"0xddcf208f219a6e6af072f2cfdc615b2c1805f98e"}) 
        with input as request
        with data.entities as entities
}

test_check_source_account_type {
    check_source_account_type({"eoa"}) 
        with input as request
        with data.entities as entities
}

test_check_source_classification {
    check_source_classification({"wallet"}) 
        with input as request
        with data.entities as entities
}

test_check_destination_address {
    check_destination_address({"0xa45e21e9370ba031c5e1f47dedca74a7ce2ed7a3"}) 
        with input as request
        with data.entities as entities
}
