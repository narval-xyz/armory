...
- EncryptionKey
  ...
  You can generate a new key by calling `POST /encryption-keys`. It will generate a new encryption key pair, and return the public key for user to encrypt with.
...
- Wallet
  It is a structure that organizes a collection of accounts. It's the abstraction around a `rootKey`. The same relationship that applies to `rootKey` and `privateKeys` applies to `wallet` and `account`.
  A `wallet` is a parent for children `accounts`.
