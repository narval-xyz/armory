...
import { deriveWallet, findNextPath, getRootKey, mnemonicToRootKey } from '../util/key-generation'
...
export class KeyGenerationService {
  ...
  private async maybeEncryptAndSaveBackup(
    clientId: string,
    kid: string,
    mnemonic: string,
    backupPublicKey?: Jwk
  ...
  private async findNextDerivationPath(clientId: string, keyId: string): Promise<string> {
    const wallets = (await this.walletRepository.findByClientId(clientId))
    return findNextPath(map('derivationPath', wallets))
  }
  ...
  async deriveWallet(clientId: string, opts: DeriveWalletDto): Promise<PrivateWallet | PrivateWallet[]> {
    ...
    const rootKey = await this.mnemonicRepository.findById(clientId, opts.rootKeyId);
    ...
    const { mnemonic } = rootKey;
    const { derivationPaths, derivations, rootKeyId: keyId } = opts;
    const wallets: PrivateWallet[] = [];

    for (let i = 0; i < derivations; i++) {
      const path = derivationPaths ? derivationPaths[i] : await this.findNextDerivationPath(clientId, keyId);
      const wallet = await deriveWallet(mnemonicToRootKey(mnemonic), { rootKeyId: keyId, path });
  ...
  async generateMnemonic(
    ...
    const { rootKey, keyId: rootKeyId } = getRootKey(mnemonic, opts.keyId)
