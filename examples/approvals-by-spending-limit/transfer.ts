import { AuthClient, AuthConfig, buildSignerEip191, privateKeyToJwk, Request, resourceId, TransactionRequest, VaultClient, VaultConfig } from '@narval/armory-sdk'
...
const main = async () => {
  ...
  if (authId) {
    ...
    switch (authRequest.status) {
      case 'APPROVING': {
        console.log('Request is waiting for approvals', JSON.stringify(auth.findApprovalRequirements(authRequest), null, 2))
        break
      }
      case 'PERMITTED': {
