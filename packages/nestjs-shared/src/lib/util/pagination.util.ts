...
import { PageCursorDecoder, PageCursorEncoder, PaginatedResult, PaginationQuery, PaginationOptions } from '../type/pagination.type'
...
export function getPaginatedResult<T extends { id: string; createdAt: Date }>({
 ...
 const data = hasNextPage && items.length > 1 
...
export const applyPagination = (pagination?: PaginationOptions): {
 skip?: number
