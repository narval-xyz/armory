...
export default class DeployContractDecoder extends DecoderStrategy {
  ...
  decode(): DeploymentIntent {
    ...
    if (fromType?.factoryType === WalletType.SAFE) {
      ...
      return { type: Intents.DEPLOY_SAFE_WALLET, from: toAccountIdLowerCase({ chainId, address: from }), chainId }
      ...
      return { type: Intents.DEPLOY_CONTRACT, from: toAccountIdLowerCase({ chainId, address: from }), chainId }
    }
  }
}
